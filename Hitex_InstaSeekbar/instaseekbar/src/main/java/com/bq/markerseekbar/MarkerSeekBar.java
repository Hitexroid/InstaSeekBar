...
import com.instaseekbar.hitexroid.instaseekbar.R;
 ...
 * <ul>
 * <p/>
 * <li>attr {@link R.styleable#MarkerSeekBar_showMarkerOnTouch}. * Automatic toggle. Default true.</li>
 * <li>attr {@link R.styleable#MarkerSeekBar_smoothTracking}. * Animate popup position. Default false.</li>
 * <li>attr {@link R.styleable#MarkerSeekBar_markerTextAppearance}. * The style of the text inside the marker.</li>
 * <li>attr {@link R.styleable#MarkerSeekBar_markerTextColor}. * The color of the text inside the marker.</li>
 * <li>attr {@link R.styleable#MarkerSeekBar_markerColor}. * The marker background color. Default accent color</li>
 * <li>attr {@link R.styleable#MarkerSeekBar_markerShadowRadius}. * The marker shadow radius. Use 0 to disable shadows. It affects marker size. Default 4dp.</li>
 * <li>attr {@link R.styleable#MarkerSeekBar_markerShadowColor}. * The marker shadow color. Default #331d1d1d.</li>
 * <li>attr {@link R.styleable#MarkerSeekBar_markerPopUpWindowSize}. * The popup size, its constant and measured to fit the longest possible text. Default 80dp.</li>
 * <li>attr {@link R.styleable#MarkerSeekBar_markerHorizontalOffset}. * Horizontal offset to align the marker tip and the progress thumb. Default empirical -8.5dp.</li>
 * <li>attr {@link R.styleable#MarkerSeekBar_markerVerticalOffset}. * Vertical offset to align the marker tip and the progress thumb. Default empirical -6dp.</li>
 * </ul>
 *
 * @see MarkerView
 */
public class MarkerSeekBar extends AppCompatSeekBar implements SeekBar.OnSeekBarChangeListener {
    ...
    public MarkerSeekBar(Context context, AttributeSet attrs) {
        ...
        final MarkerSeekBar markerSeekBar = this;
        postDelayed(new Runnable() {
            @Override
            public void run() {
                onGradientDraw(markerSeekBar, getProgress());
                markerTextView.setTextColor(Color.WHITE);
                markerTextView.setTextSize(14);
            }
        }, 100);
    }

    private void onGradientDraw(SeekBar seekBar, int value) {
        if (seekBar.getThumb() instanceof StateListDrawable) {
            StateListDrawable listDrawable = (StateListDrawable) seekBar.getThumb();
            LayerDrawable layerDrawable = (LayerDrawable) listDrawable.getCurrent();
            GradientDrawable gradientDrawable = (GradientDrawable) layerDrawable.findDrawableByLayerId(R.id.cg);
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        onGradientDraw(seekBar, progress);
    ...
    public void onStopTrackingTouch(final SeekBar seekBar) {
        if (showMarkerOnTouch) {
            seekBar.postDelayed(new Runnable() {
                @Override
                public void run() {
                    onGradientDraw(seekBar, seekBar.getProgress());
                }
            }, 100);
            hideMarker(true, 0);
        }
